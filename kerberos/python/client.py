...
def access_authentication_service(client_name, client_ip, client_to_authentication_service_timestamp):
    ...
    request = ClientToAuthenticationServiceSession().generate_session(client_name, client_ip, client_to_authentication_service_timestamp)
...
def access_ticket_granting_service(encrypted_ticket_granting_service_ticket_base64, client_name, client_to_ticket_granting_service_session_key, server_ip, client_ip):
    ...
    client_to_ticket_granting_service_session = ClientToTicketGrantingServiceSession(client_to_ticket_granting_service_session_key).generate_session(client_name, client_ip, client_to_ticket_granting_service_timestamp)
    ...
    ticket_granting_service_request = f"{encrypted_ticket_granting_service_ticket_base64},{server_ip},{client_to_ticket_granting_service_session}"
    ...
    try:
        ticket_granting_service_to_client_timestamp, biz_service_ticket_validity, client_to_biz_service_session_key =  TicketGrantingServiceToClientSession(client_to_ticket_granting_service_session_key).parse_session(encrypted_session)
    except:
        print("Error parsing decrypted encrypted_session: ", encrypted_session)
        return None, None, None
    
    return encrypted_biz_service_ticket_base64, client_to_biz_service_session_key, biz_service_ticket_validity
    

def access_biz_service(encrypted_biz_service_ticket_base64, client_to_biz_service_session_key, client_name, client_ip, biz_service_ticket_validity):
    # Create Authenticator
    client_to_biz_service_timestamp = str(int(time.time()))
    encrypted_client_to_biz_service_session = ClientToBizServiceSession(client_to_biz_service_session_key).generate_session(client_name, client_ip, client_to_biz_service_timestamp, biz_service_ticket_validity)
...
def main():
    ...
    encrypted_ticket_granting_service_ticket_base64, authentication_service_to_client_timestamp, client_to_ticket_granting_service_session_key = \
    ...
    encrypted_biz_service_ticket_base64, client_to_biz_service_session_key, biz_service_ticket_validity = \
    ...
    print(f"encrypted_biz_service_ticket: {encrypted_biz_service_ticket_base64}, client_to_biz_service_session_key: {client_to_biz_service_session_key}, biz_service_ticket_validity: {biz_service_ticket_validity}")
    # Step 3: Access the service
    server_response = access_biz_service(encrypted_biz_service_ticket_base64, client_to_biz_service_session_key, client_name, client_ip, biz_service_ticket_validity)
